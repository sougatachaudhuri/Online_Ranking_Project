            ...
            if ~strcmp(nqid,qid)
                if(q~=0)
                    %if (rand<0.5) % Will allow the selected query to be processed with probability 0.5, so as to avoid repetition of seeing queries in order over every run
                        ...
                        % Update the rankers   
                        eta=0.001/(nthroot(count^2,3)); % Another RTop-1F parameter,using theoretically derived value, but can be tuned.
                        if(count==1)
                        ...
                        wranktemp=wrankListNet - (0.01/sqrt(count))*gradListNet; % 0.1/sqrt(count) is the ListNet learning parameter. Can be tuned.
